...
import { asArray, atualizarPedidoStatus, getBebidaIds, getPedidosAbertos, imprimirItensPedido, itemNovoParaImpressao, itensNovosParaImpressao, marcarItensComoImpressos, removerItemPedido } from '../services/api.js';
...
export default function BalcaoPedido() {
  ...
  const [bebidaIds, setBebidaIds] = useState(null);
  ...
  useEffect(() => {
    async function loadPedido() {
      ...
      try {
        ...
        const atual = pedidos.find((item) => String(getPedidoId(item)) === String(id) && pedidoAberto(item));
  ...
  const itensNovos = useMemo(() => itensNovosParaImpressao(pedidoIdAtual, itens, bebidaIds), [pedidoIdAtual, itens, bebidaIds]);
  const total = useMemo(() => {
  ...
  async function updateStatus(status) {
    ...
    if (!window.confirm(`Confirmar ${label} o pedido #${pedidoId}?`)) return;
    ...
    try {
      ...
      navigate('/balcao/pedidos', { replace: true });
    } catch (err) {
  ...
  async function imprimirPedido() {
    ...
    const novos = itensNovosParaImpressao(pedidoId, itens, bebidaIds);
    if (!novos.length) {
      setError('');
  ...
  return (
    <main className="app-shell">
      <div className="mobile-page">
        <Header title={`Pedido #${id}`} showBack />
        {!loading && pedido ? (
        ...
        {!loading && !pedido ? (
          <section className="pt-10">
            <div className="card text-center">
              <p className="text-xl font-bold text-white">Pedido não encontrado</p>
              <p className="mt-2 text-gray-400">Ele pode ter sido fechado ou cancelado.</p>
            </div>
            <button className="primary-button mt-5 w-full text-xl" type="button" onClick={() => navigate('/balcao/pedidos', { replace: true })}>
              Voltar aos pedidos
            </button>
          </section>
        ) : null}
          ...
          <section className="space-y-4 pb-40">
            ...
            <div className="fixed inset-x-0 bottom-0 z-20 mx-auto max-w-[480px] border-t border-purple-500/25 bg-black/95 p-4 shadow-2xl shadow-purple-950/30 backdrop-blur">
              ...
              <div className="grid grid-cols-1 gap-3">
                <button className="primary-button" type="button" onClick={() => navigate(`/balcao/pedido/${id}/adicionar`)}>
                  Adicionar Itens
                </button>
